...
List<int> SubBytes(List<int> state) {
  List<int> kq = List.filled(4, 0);
  for (int i = 0; i < 4; i++) kq[i] = subWord(state[i]);
  //           stdout.write("\nSubWord\n");
  //         for(int i = 0 ; i < 4; i++){
  //             stdout.write("\n\t");
  //             ShowWord(kq[i]);
...
List<int> InvMixColumns(List<int> state) {
  List<int> kq = List.filled(4, 0);
  for (int i = 0; i < 4; i++) kq[i] = InvNhanCot(state[i]);
  return kq;
}
...
void main() {
  ...
  List<int> Key = [w0, w1, w2, w3];
  int state0 = 0x3243f6a8;
  int state1 = 0x885a308d;
  int state2 = 0x313198a2;
  int state3 = 0xe0370734;
  List<int> state = [state0, state1, state2, state3];
